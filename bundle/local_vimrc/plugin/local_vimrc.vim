...
let g:local_vimrc = s:LocalVimrcName()
...
function! s:SourceLocal(path)
  ...
  if (a:path !~ s:re_last_path)
    ...
    elseif filereadable(up_path.'/'.g:local_vimrc)
      " Terminal condition: try to source the upper(/uppest?) local-vimrc
      if &verbose >= 2
	echo 'Check '.up_path.' for '.g:local_vimrc.' ... found!'
      endif
      exe 'source '.escape(up_path.'/'.g:local_vimrc, ' \$,')
    elseif &verbose >= 2
      echo 'Check '.up_path.' for '.g:local_vimrc.' ... none!'
    endif
  endif

  " In all case, try to source the local-vimrc present in the directory
  " currently considered
  if filereadable(a:path.'/'.g:local_vimrc)
    if &verbose >= 2
      echo 'Check '.a:path.' for '.g:local_vimrc.' ... found!'
    endif
    exe 'source '.escape(a:path.'/'.g:local_vimrc, ' \$,')
  elseif &verbose >= 2
    echo 'Check '.a:path.' for '.g:local_vimrc.' ... none!'
  endif
endfunction
...
aug LocalVimrc
  ...
  exe 'au BufWritePre '.g:local_vimrc . ' call s:IncrementVersionOnSave()'
aug END
